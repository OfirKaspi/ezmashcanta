...
export async function GET(req: Request) {
  ...
  if (CRON_SECRET && CRON_SECRET !== "" && req.headers.get("Authorization") !== `Bearer ${CRON_SECRET}`) {
  ...
  if (!ZOHO_CLIENT_ID || !ZOHO_CLIENT_SECRET || !ZOHO_REFRESH_TOKEN || !ZOHO_REDIRECT_URI) {
    return NextResponse.json(
      { success: false, message: "Missing Zoho configuration. Please set ZOHO_CLIENT_ID, ZOHO_CLIENT_SECRET, ZOHO_REFRESH_TOKEN, and ZOHO_REDIRECT_URI in your .env file." },
      { status: 500 }
    )
  }
  ...
  if (!UPSTASH_REDIS_REST_URL || !UPSTASH_REDIS_REST_TOKEN) {
    return NextResponse.json(
      { success: false, message: "Missing Upstash Redis configuration. Please set UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN in your .env file." },
      { status: 500 }
    )
  }
